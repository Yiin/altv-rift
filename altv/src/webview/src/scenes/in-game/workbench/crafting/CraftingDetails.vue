  ...
  <div class="flex flex-col items-end text-right">
    <template v-if="selectedRecipe">
      ...
      <div class="mb-12 mt-5 flex flex-wrap items-center justify-end gap-2">
        <WorkbenchSlot v-for="(part, index) in selectedRecipe.parts" :key="`${part.key}-${index}`"
          class="relative h-28 w-28" :selected="false">
          <ItemIcon class="mt-5" :class="{ 'opacity-50': !hasMatchingPart(part, character.inventory) }" :item="part"
      ...
      <div class="mb-18.75 flex w-56.25 grow flex-col justify-end">
        ...
        <button v-if="isCurrentlyBeingCrafted" class="button mt-2.5 p-4 pt-3.5 text-center text-lg font-bold"
          @click="() => cancelCrafting()">
          Cancel crafting
        </button>
        <button v-else-if="isInQueue" class="button mt-2.5 p-4 pt-3.5 text-center text-lg font-bold"
          @click="() => removeFromQueue()">
          Remove from queue
        </button>
        <button v-else class="button mt-2.5 p-4 pt-3.5 text-center text-lg font-bold"
          @click="() => onStartCrafting(quantity)" :disabled="!canCraftSelectedRecipe">
          Begin crafting
        </button>
      </div>
    </template>
...
<style scoped>
.button {
  background: url("/assets/workbench/button-background.svg");
  background-size: contain;
}

.button:not(:disabled):hover {
  background: url("/assets/workbench/button-background-hovered.svg");
  background-size: contain;
}
