...
import { type MatchedCommand, type CommandSuggestion } from "@/interfaces";
...
const { focus, message, setMessage, commandSuggestions, setCommandSuggestions, options } =
  useChatStore();
...
function selectCommand(event: KeyboardEvent) {
  ...
  if (event.key === "ArrowUp" && matchedCommands.value.length > 1)
    ...
    event.key === "Tab" &&
    selected.value >= 0 &&
    !(matchedCommands.value[selected.value].currentParam > -1)
  )
...
watch([message, commandSuggestions, options], ([message, commands]) =>
...
<template>
  <div
    class="mt-[4px] flex origin-top scale-y-0 flex-col text-white transition"
    :class="{
  ...
  >
    <div
      ...
      :key="matchedCommand.name"
      class="select-none bg-black px-[16px] py-[8px] transition duration-200"
      :class="{
