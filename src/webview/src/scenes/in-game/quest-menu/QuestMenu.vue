...
const defaultPosition = computed(() => {
  return player.user?.interface?.questMenu?.screen ?? { x: window.innerWidth / 4, y: window.innerHeight / 3, w: 400 };
});
...
const quests = computed(() => {
  switch (filter.value) {
    case 'incomplete':
      return client.activeQuests.filter(quest => quest.tasks.some(task => !questFacts.value.includes(task.completedFact)));
    case 'complete':
      return client.activeQuests.filter(quest => quest.tasks.every(task => questFacts.value.includes(task.completedFact)));
    case 'all':
