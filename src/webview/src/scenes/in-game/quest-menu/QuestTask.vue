...
import { QuestTaskInfo } from "@shared/store/client.store";
...
<template>
  <details :class="[
    ...
    <summary @click="toggleSingle" class="flex cursor-pointer items-center justify-between gap-1.5 text-white">
      <h2 class="text-lg font-medium">
        {{ task.title }}
      </h2>

      <span class="relative h-5 w-5 shrink-0">
        <svg xmlns="http://www.w3.org/2000/svg" class="absolute inset-0 h-5 w-5 opacity-100 group-open/task:opacity-0"
          fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round"
            d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>

        <svg xmlns="http://www.w3.org/2000/svg" class="absolute inset-0 h-5 w-5 opacity-0 group-open/task:opacity-100"
          fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div v-if="!isCompleted" class="mt-3">
      <button @click="trackQuest(task.completedFact)"
        class="inline-flex items-center gap-2 rounded border px-2 py-1 uppercase text-white focus:outline-none"
        :class="[client.trackingQuest === task.completedFact ? 'border-emerald-600 bg-emerald-600 hover:text-emerald-600' : 'border-neutral-600 bg-neutral-600 hover:text-neutral-600']">
        <span class="text-xs font-medium">
          {{ client.trackingQuest === task.completedFact ? 'Tracking' : 'Track' }}
        </span>

        <v-icon icon="mdi-map-marker-plus-outline" size="xs" />
      </button>
    </div>
  </details>
</template>
