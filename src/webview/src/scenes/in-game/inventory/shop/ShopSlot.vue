...
import { useInventory, InteractionType, isSameSource, SlottedItem, } from "@/store/inventory.store";
...
const draggingStyle = computed(() => {
  ...
  const slotPositionInGrid = inventory.getItemSourceRelativePosition(item.value.source);
