...
import { MatchedCommand, CommandSuggestion } from "@/interfaces";
 ...
 */
function updateMatchedCommands(
  message: string,
  commands: Array<CommandSuggestion>
) {
  if (!message) {
  ...
  const cmdName = words[0].startsWith(options.prefix)
    ? words[0].substring(1)
    : "";
    ...
    .filter(
      (command) =>
        (command.name === cmdName &&
          cmdParams.length <= (command.parameters?.length ?? 0)) ||
      ...
      <div class="text-xs text-white text-opacity-50">
        {{
          (matchedCommand.currentParam <= 0
            ...
            : matchedCommand.parameters![matchedCommand.currentParam - 1]
                .description) ?? ""
        }}
      </div>
    </div>
