...
export const useInventory = defineStore("inventory", {
  ...
  actions: {
    equipItem(slot: number) {
      return rpc.callClient<boolean>(RPC.Client.EQUIP_ITEM, slot);
    },
    dropItem(slot: number) {
      return rpc.callClient<boolean>(RPC.Client.DROP_ITEM, slot);
    },
    async moveItem(from: number, to: number, local = false) {
      ...
      const ok = await rpc.callClient<boolean>(RPC.Client.MOVE_ITEM, from, to);
