import { isConsoleOpen, log } from "alt-client";
...
export class Chat {
  ...
  public start() {
    this.eventService.onServer(
      "vchat:toggleVisibility",
      this.toggleWindowVisibility.bind(this)
    );
    this.eventService.onServer(
      "vchat:toggleFocus",
      this.toggleWindowFocus.bind(this)
    );
    this.eventService.onServer(
    ...
    this.eventService.onServer(
      "vchat:addMessage",
      this.addMessageToWindow.bind(this)
    );
    this.eventService.onServer(
      "vchat:addSuggestion",
      this.addSuggestionToWindow.bind(this)
    );
    this.eventService.onServer(
    ...
    this.eventService.onServer(
      "vchat:clearMessages",
      this.clearWindowMessages.bind(this)
    );
    this.eventService.onServer(
      "vchat:clearMessageHistory",
    ...
    this.eventService.on("keyup", this.toggleWindowFocusOnKey.bind(this));

    this.windowService.on(
      "vchat:requestSettings",
      this.requestSettings.bind(this)
    );
  ...
  public addMessageToWindow(
    message: string,
    type: MessageType = MessageType.Default
  ) {
    if (
      this.messageHistoryService.getLength() >
      this.optionsService.get("maxMessageHistory")
    )
  ...
  public syncSettings(
    ...
    this.windowService.syncSettings(
  ...
  public toggleWindowFocusOnKey(key: number) {
    if (isConsoleOpen()) return;
    if (this.optionsService.get("unfocusKey") === key)
      this.windowService.unfocus();
    else if (this.optionsService.get("focusKey") === key)
