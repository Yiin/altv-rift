...
import { IconName } from "@/modules/rmlui/components/icon/icon";
...
registerNpcInteractions(Npc.CAL_BURNETT, (ped: alt.Ped) => {
  ...
  const questFacts = playerStore.character?.questFacts ?? [];
...
registerNpcInteractions(Npc.DIEGO_MOREIRA, (ped) => {
  ...
  if (
    [
      ...
      (fact) =>
        questFacts.includes(fact) && !questFacts.includes(Quests.Introduction.Facts.COMPLETED_ALL)
    )
  ) {
    interactions.push({
      icon: "quest",
    ...
    questFacts.includes(Quests.Introduction.Facts.GOT_DIRECTIONS) &&
    !questFacts.includes(Quests.Introduction.Facts.COMPLETED_ALL)
  ) {
    interactions.push({
      icon: "quest",
      label: "Talk",
        ...
        startConversation(ped, {
          ...
          startConversation(ped, {
            ...
            if (option?.value === "complete") {
              alt.emitServerRaw(ServerEvents.FromClient.NOTIFY, selectedOption.value);
            }
          });
        });
      },
