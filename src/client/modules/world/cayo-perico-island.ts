...
alt.setInterval(() => {
  alt.log(`Distance: ${alt.Player.local.pos.distanceTo(islandCenter)}`);
  alt.log(islandArea.isEntityIn(alt.Player.local) ? `In Island` : `Not In Island`);

  if (alt.Player.local.pos.distanceTo(islandCenter) < 3000) {
    if (!nearIsland) {
      loadIsland();
    }
  } else {
