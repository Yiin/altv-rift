...
export default class FlyController {
  ...
  private constructor() {
    ...
    alt.Events.onGameEntityCreate(this.onGameEntityCreate);
  }

  setFlyStatus(status: boolean) {
    FlyController.isFlying = status;
    natives.freezeEntityPosition(alt.Player.local.id, status);
  }

  onGameEntityCreate = ({ entity }: alt.Events.GameEntityCreateEventParameters) => {
    if (!(entity instanceof alt.Player)) return;
    natives.freezeEntityPosition(entity.id, FlyController.isFlying);
  };
  ...
  private _everyTick?: alt.Timers.EveryTick;
  private _speed = 1;
  private _cam?: number;
  ...
  start(freecam = false) {
    ...
    this._state = true;
    if (freecam) {
      this._cam = natives.createCamWithParams(
        "DEFAULT_SCRIPTED_CAMERA",
        player.pos.x,
        player.pos.y,
        player.pos.z + 1,
        player.rot.x * Utils.Rad2Deg,
        player.rot.y * Utils.Rad2Deg,
    ...
    this._everyTick = alt.Timers.everyTick(freecam ? this.handleFreecam : this.handle);
  }
  ...
  getNewPos(pos: alt.IVector3) {
    ...
    if (natives.isDisabledControlPressed(0, 241)) {
      // INPUT_CURSOR_SCROLL_UP
      this._speed = Utils.clamp(this._speed + 0.1, 0.1, 10);
    }

    if (natives.isDisabledControlPressed(0, 242)) {
    ...
    const rr = new alt.Vector3(rot).toRadians().radiansToDirection();
    const preRightVector = new alt.Vector3(rr.normalized).cross(upVector);
