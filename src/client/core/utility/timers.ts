...
const timersData: { [key: number]: number[] } = {};
const timeouts: TimerInfo[] = [];
let count;
...
export const Timer = {
  ...
  getTimersInfo() {
    alt.log("=== TIMERS INFO ===");
    for (const [smallMS = 0, largeMS = 0] of Object.values(TIMER_CATEGORIES)) {
      const validTimers = timers.filter(
        (x) => x.ms >= smallMS && x.ms <= largeMS
      );
