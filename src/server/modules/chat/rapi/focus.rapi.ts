...
export function toggleFocusEnabled(player: Player, enabled: boolean) {
  container
    ...
    .waitForMount(
      player,
      container.resolve(WindowService).toggleFocusEnabled(player, enabled)
    );
}
...
export function focus(player: Player) {
  container
    .resolve(MountService)
    .waitForMount(player, container.resolve(WindowService).focus(player));
}
...
export function unfocus(player: Player) {
  container
    .resolve(MountService)
    .waitForMount(player, container.resolve(WindowService).unfocus(player));
}
