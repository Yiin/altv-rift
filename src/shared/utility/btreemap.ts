...
export class BTreeMap<T = any> implements IBTreeMap<T> {
  #unique: boolean = true;
  #order: number = 3;
  ...
  #headers: any = [
    {
      length: 8,
      get: () => this.#order,
      set: (v: number) => (this.#order = v),
    },
    {
      length: 1,
      get: () => (this.#unique === false ? 0 : 1),
      set: (v: number) => (this.#unique = v === 0 ? false : true),
    },
  ];
